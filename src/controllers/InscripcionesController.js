...
export default class InscripcionesController {
  ...
  static async getGestionInscripciones(req, res) {
    try {
      ...
      return respuestaAlFront(
        res,
        "error",
        "Error interno al cargar gestión",
        {},
        500,
      );
    }
  ...
  static async buscarEstudiante(req, res) {
    try {
      ...
      const estudiante =
        await InscripcionModel.buscarEstudiantePorCedula(cedula);
    ...
    try {
      ...
      if (!idUsuarioFinal || !materia_id || !seccion_id) {
        return respuestaAlFront(
          res,
          "error",
          "Faltan datos obligatorios para procesar la inscripción",
          {},
          400,
        );
      }
      ...
      if (estudiante_id) {
        ...
        return respuestaAlFront(
          res,
          "ok",
          "Inscripción registrada con éxito por el administrador",
          {
        ...
        return respuestaAlFront(
          res,
          "ok",
          "Tu solicitud de inscripción ha sido enviada",
          {
      ...
      return respuestaAlFront(
        res,
        "error",
        "No se pudo procesar la operación en la base de datos",
        {},
        500,
      );
    }
  ...
  static async actualizarEstadoSolicitud(req, res) {
    try {
      ...
      const actualizado = await InscripcionModel.actualizarEstado(
        id,
        estado,
        comentario,
      );
