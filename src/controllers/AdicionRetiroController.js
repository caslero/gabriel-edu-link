...
export class AdicionRetiroController {
  ...
  static async buscarEstudiante(req, res) {
    try {
      ...
      const estudiante =
        await AdicionRetiroModel.buscarEstudiantePorCedula(cedulaLimpia);
  ...
  static async listarSemestres(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ status: "error", message: "Error al obtener semestres" });
    }
  }
  ...
  static async listarMaterias(req, res) {
    try {
      ...
      const materias =
        await AdicionRetiroModel.obtenerMateriasPorSemestre(semestre);
      return res.json({ status: "ok", materias });
    } catch (error) {
      return res
        .status(500)
        .json({ status: "error", message: "Error al obtener materias" });
    }
  }
  ...
  static async listarSecciones(req, res) {
    try {
      ...
      const secciones =
        await AdicionRetiroModel.obtenerSeccionesPorNombreMateria(
          nombreMateria,
        );
      ...
      return res
        .status(500)
        .json({ status: "error", message: "Error al obtener secciones" });
    }
  }
