...
export class InscripcionModel {
  ...
  static async crearInscripcion(estudiante_id, seccion_id, usuario_id) {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        db.run(
          sqlInscripcion,
          [estudiante_id, seccion_id, usuario_id],
          function (err) {
            if (err) {
    ...
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        db.get(sqlGetSeccion, [id], (err, row) => {
          ...
          db.run(
            sqlUpdate,
            [estado, comentario, usuario_id, id],
            function (err) {
              if (err) {
                db.run("ROLLBACK");
                return reject(err);
              ...
              if (estado === "Aprobada") {
                ...
                db.run(sqlCupo, [seccion_id], function (err) {
                  if (err || this.changes === 0) {
                    ...
                    return reject(
                      new Error(
                        "No hay cupos disponibles para aprobar esta solicitud",
                      ),
                    );
                  }
                  db.run("COMMIT");
