document.addEventListener("DOMContentLoaded", () => {
    ...
    semestreSelect.addEventListener("change", async (e) => {
      ...
      try {
        const res = await fetch(
          `/api/adicion-retiro/materias?semestre=${semestre}`,
        );
        const data = await res.json();
        if (data.status === "ok") {
  ...
  if (materiaSelect) {
    materiaSelect.addEventListener("change", async (e) => {
      ...
      try {
        const res = await fetch(
          `/api/adicion-retiro/secciones?nombreMateria=${encodeURIComponent(nombreMateria)}`,
        );
        const data = await res.json();
        if (data.status === "ok") {
  ...
  if (formNueva) {
    formNueva.addEventListener("submit", async (e) => {
      ...
      if (
        isNaN(payload.estudiante_id) ||
        isNaN(payload.seccion_id) ||
        !payload.tipo
      ) {
  ...
  function resetCascada() {
    estudianteSelect.innerHTML =
      '<option value="">Realice una búsqueda...</option>';
...
async function gestionarSolicitud(id, nuevoEstado) {
  if (!confirm(`¿Estás seguro de marcar esta solicitud como ${nuevoEstado}?`))
    return;
  try {
