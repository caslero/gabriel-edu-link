...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- LÓGICA DE VER/OCULTAR CONTRASEÑA REUTILIZABLE ---
  ...
  // Botones atrás
  if (step2Prev) step2Prev.onclick = () => { step2.classList.add("hidden"); step1.classList.remove("hidden"); activarPaso(1); };
  if (step3Prev) step3Prev.onclick = () => { step3.classList.add("hidden"); step2.classList.remove("hidden"); activarPaso(2); };
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch("/api/usuario/crear-usuario", {
        method: "POST",
      ...
      if (resultado.status === "ok") {
        ...
        setTimeout(() => {
          window.location.href = resultado.data.redirect || "/login";
        }, 2000);
      } else {
