...
async function fetchMaterias(semestre) {
  try {
    const res = await fetch(
      `/api/encuestas/listar-materias?semestre=${semestre}`,
    );
...
function renderMateriasChecks(materias, seleccionadasIds = []) {
  if (materias.length === 0)
    return '<p class="text-gray-500 text-sm">No hay materias disponibles.</p>';
...
async function enviarEncuesta(url, metodo, data) {
  try {
    ...
    if (result.status === "ok") {
      mostrarNotificacion(
        metodo === "POST" ? "Encuesta creada" : "Encuesta actualizada",
      );
...
window.abrirModalEditar = async function (id) {
  ...
  try {
    ...
    if (result.status === "ok") {
      ...
      document.getElementById("edit-materias-list").innerHTML =
        renderMateriasChecks(todasLasMaterias, seleccionadasIds);
    }
  } catch (err) {
  ...
  try {
    ...
    if (result.status === "ok") {
      ...
      if (encuestas.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="7" class="text-center py-6 text-gray-500">No hay encuestas registradas.</td></tr>';
        return;
      }

      tbody.innerHTML = encuestas
        .map((encuesta) => {
            ...
            encuesta.estado === "Activa"
              ...
              : encuesta.estado === "Pendiente"
                ...
                : encuesta.estado === "Finalizada"
                  ? "bg-blue-100 text-blue-700" // Color para finalizada
...
window.verResultados = async function (id) {
  ...
  modal.classList.remove("hidden");
  ganadoresCont.innerHTML =
    "<p class='text-center col-span-3'>Cargando resultados...</p>";
  listaCont.innerHTML = "";
  ...
  try {
    ...
    if (result.status === "ok") {
      ...
      document.getElementById("res-titulo").textContent =
        `Resultados: ${result.encuesta_titulo}`;
      ...
      if (resto.length > 0) {
        ...
        ganadoresCont.innerHTML =
          "<p class='text-center col-span-3 text-gray-500'>Aún no hay votos registrados.</p>";
      }
    }
  } catch (err) {
    console.error("Error al obtener resultados:", err);
    ganadoresCont.innerHTML =
      "<p class='text-center col-span-3 text-red-500'>Error al cargar los datos.</p>";
  }
};
...
function crearCardEncuesta(encuesta) {
  ...
  const listaMaterias = encuesta.materias
    .map((m) => `<li>• ${m.nombre}</li>`)
    .join("");
...
//
//
/** Esta es la funcion de obtener las encuestas, inicia sesion con algun usuario que
 Tenga encuestas a ver si funciona y de paso compara esta funcion con la de mas abajo
 que esta comentada
 */
...
async function validarVotos(encuestaId, form) {
  ...
  try {
    ...
    if (result.status === "ok") {
      ...
      Swal.fire(
        "Error",
        result.message || "No se pudo registrar el voto",
        "error",
      );
    }
  } catch (error) {
