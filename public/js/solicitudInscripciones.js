document.addEventListener("DOMContentLoaded", () => {
  ...
  if (formInscripcion) {
    formInscripcion.addEventListener("submit", async (e) => {
      ...
      const selectSeccion = formInscripcion.querySelector(
        '[name="seccion_id"]',
      );

      // Log de diagnóstico: ¿Qué está leyendo realmente?
      console.log("Diagnóstico de campos:", {
        valorUsuarioRaw: inputUsuario
          ? inputUsuario.value
          : "NO EXISTE EL INPUT",
        valorSeccionRaw: selectSeccion
          ? selectSeccion.value
          : "NO EXISTE EL SELECT",
      });
      ...
      if (!inputUsuario || !selectSeccion) {
        return mostrarNotificacion(
          "Error técnico: Campos no encontrados",
          "error",
        );
      }
      ...
      if (isNaN(payload.usuario_id)) {
        console.error(
          " Fallo crítico: usuario_id es NaN. El servidor envió un valor vacío al HTML.",
        );
        return mostrarNotificacion(
          "Error: ID de usuario no válido. Revisa tu sesión.",
          "error",
        );
      }
      ...
      try {
        const btnSubmit = formInscripcion.querySelector(
          'button[type="submit"]',
        );
        if (btnSubmit) {
